...
export function colorToRGBA(color) {
  ...
  // Extract each channel as integers
  const red = parseInt(color.slice(1, 3), 16);   // First 2 chars after #
  const green = parseInt(color.slice(3, 5), 16); // Next 2 chars
  const blue = parseInt(color.slice(5, 7), 16);  // Next 2 chars
  const alpha = parseInt(color.slice(7, 9), 16); // Last 2 chars

  return `rgb(${red} ${green} ${blue} / ${Math.round(alpha / 255 * 1000) / 1000})`;
}